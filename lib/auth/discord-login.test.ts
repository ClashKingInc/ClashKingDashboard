...
describe('initiateDiscordLogin', () => {
  ...
  it('initiates tunneled login with the configured public ID and exact callback', async () => {
    vi.stubEnv('VITE_DISCORD_CLIENT_ID', '808566437199216691');
    Object.defineProperty(window, 'location', {
      value: { origin: 'https://local-dash.clashk.ing', hostname: 'local-dash.clashk.ing', href: '' },
      configurable: true,
    });
    await initiateDiscordLogin('en');
    const authorization = new URL(window.location.href);
    expect(authorization.origin).toBe('https://discord.com');
    expect(authorization.searchParams.get('client_id')).toBe('808566437199216691');
    expect(authorization.searchParams.get('redirect_uri')).toBe('https://local-dash.clashk.ing/auth/callback');

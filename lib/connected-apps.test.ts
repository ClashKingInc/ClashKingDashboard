...
import { buildConnectReturnUrl, readConnectRequest } from "./connected-apps";
...
describe("readConnectRequest", () => {
  it("reads the permanent application URL and preserves opaque state", () => {
    expect(readConnectRequest(new URL(
      "https://dash.clashk.ing/connect/app_123?redirect_uri=https%3A%2F%2Fexample.com%2Fcallback%3Fsource%3Dck&state=a%2Bb%26c",
    ))).toEqual({
  ...
  it("allows a permanent URL without redirect state", () => {
    expect(readConnectRequest(new URL("https://dash.clashk.ing/connect/app_123"))).toEqual({
      applicationId: "app_123",
    });
  });

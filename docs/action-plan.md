...
### 1. Add `state` parameter to the Discord OAuth flow — S
...
### 2. Add security headers in `next.config.ts` — S/M
...
### 4. Server-side auth guard for `/dashboard` and `/servers` — M
...
### 6. Only one `loading.tsx` in the whole app — S
...
### 7. Fetches in `useEffect` without cancellation — M
...
### 8. Proxy routes assume the backend answers JSON — M
...
### 9. Hard redirect to login from the SDK refresh path — S
...
### 10. Native `alert()` used for validation & errors — S
...
### 11. Hardcoded English error strings in localized pages — M
...
### 12. Collapse the 88 copy-pasted proxy routes — M
...
### 13. Adopt TanStack Query for dashboard data — L
...
### 14. Split the monolithic pages — L (incremental)
...
### 15. Reduce `any` usage — M (incremental)
...
### 16. Remove dead code & dependencies — S
...
### 18. Extend test coverage to dashboard logic — M (incremental)
...
### 20. Replace `<img>` with `next/image` — S
...
### 21. Dynamic server count on the landing page — S

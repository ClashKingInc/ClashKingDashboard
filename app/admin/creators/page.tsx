...
export default function CreatorApplicationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const response = await apiFetch(`/v2/admin/stats/creator-applications?status=${status}`, {
  ...
  async function review(item: CreatorApplication, nextStatus: "approved" | "rejected") {
    ...
    try {
      const response = await apiFetch(`/v2/admin/stats/creator-applications/${encodeURIComponent(item.user_id)}`, {
        method: "PATCH",
